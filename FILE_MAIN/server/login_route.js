...
router.post('/addLogin',async(req,res)=>{
    if(req.files === null){
        return res.json({data : 'No File'})
    }
    const file = req.files.file
    // console.log(`${__dirname}/../../client/public/images/${file.name}`)
    file.mv(`${__dirname}/../../client/public/images/${req.body.filename}.jpg`)

    const newLoginInfo = new loginInfo({
        name : req.body.name,
        email : req.body.email,
        pwd : req.body.pwd,
        photo : req.body.filename + ".jpg"
    })
    newLoginInfo.save()
})
...
router.delete('/deleteOneLogin/:id',async(req,res)=>{
    let result = await loginInfo.findById(req.params.id)
    console.log(result)
    if(result){
        var fs = require('fs')
        fs.unlinkSync(`${__dirname}/../../client/public/images/${result.photo}`)
    }

    result = await loginInfo.findByIdAndDelete(req.params.id)
    res.json(result)
})
